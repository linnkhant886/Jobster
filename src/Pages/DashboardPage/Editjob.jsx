...
import { EditingJob, resetIsJobUpdated } from "../../Features/Job/Job";
...
const Editjob = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { position, company, jobLocation, status, jobTypes, _id } = job;
    console.log(job);
    console.log("jobId:", _id);
    console.log("job:", { position, company, jobLocation, status, jobTypes });
    ...
    dispatch(
      EditingJob({
        jobId: _id,
        job: { position, company, jobLocation, status, jobTypes },
      })
    );
  };
  ...
  useEffect(() => {
    if (isJobUpdated) {
      navigate("/all-job");
      dispatch(resetIsJobUpdated());
    }
  }, [isJobUpdated, navigate, dispatch]);
  return (
    <div className="  p-10 shadow-2xl  w-full   ">
      <div className="   bg-white py-20 px-24 rounded-lg ">
        <h1 className="text-3xl  font-mono text-center  mb-10">Edit Job</h1>
